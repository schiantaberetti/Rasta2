...
//--------------------------------------------------------------------------
static int CheckFileSkip(ImageInfo_t* ImageInfo)
{
...
//--------------------------------------------------------------------------
static void DoFileRenaming(const char * FileName,ImageInfo_t* ImageInfo)
{
    int NumAlpha = 0;
    int NumDigit = 0;
...
//--------------------------------------------------------------------------
static int DoAutoRotate(const char * FileName,ImageInfo_t* ImageInfo)
{
    if (ImageInfo->Orientation >= 2 && ImageInfo->Orientation <= 8){
        const char * Argument;
        Argument = ClearOrientation(ImageInfo);
...
//--------------------------------------------------------------------------
static int RegenerateThumbnail(const char * FileName,ImageInfo_t *ImageInfo)
{
    ...
    if (system(ThumbnailGenCommand) == 0){
        // Put the thumbnail back in the header
        return ReplaceThumbnail(FileName,ImageInfo);
    }else{
...
void ProcessFile(const char * FileName,ImageInfo_t** ImageInfo)
{
    ...
    *ImageInfo = (ImageInfo_t*) malloc (sizeof(ImageInfo_t));
    ...
    if (ApplyCommand || AutoRotate){
        ...
        if (!ReadJpegFile(FileName, READ_METADATA,*ImageInfo)) return;
        ...
        if (AutoRotate){
            if (DoAutoRotate(FileName,*ImageInfo)){
                Modified = TRUE;
            }
        }else{
            struct stat dummy;
        ...
        if(!ReadJpegFile(RelativeExifName, READ_METADATA,*ImageInfo)) return;
    ...
    if (!ReadJpegFile(FileName, ReadMode,*ImageInfo)) return;
    ...
    if (ShowConcise){
        ShowConciseImageInfo(*ImageInfo);
    }else{
    ...
    if (ThumbSaveName){
        ...
        if (SaveThumbnail(OutFileName,*ImageInfo)){
            printf("Created: '%s'\n", OutFileName);
        }
    }
    ...
    if (CreateExifSection){
        ...
        create_EXIF(*ImageInfo);
        Modified = TRUE;
    }

    if (RegenThumbnail){
        if (RegenerateThumbnail(FileName,*ImageInfo)){
            Modified = TRUE;
        }
    }
    ...
    if (ThumbInsertName){
        ...
        if (ReplaceThumbnail(ThumbFileName,*ImageInfo)){
            Modified = TRUE;
        }
    }else if (TrimExif){
        // Deleting thumbnail is just replacing it with a null thumbnail.
        if (ReplaceThumbnail(NULL,*ImageInfo)){
            Modified = TRUE;
        }
    }
    ...
    if (ExifTimeAdjust || ExifTimeSet || DateSetChars || FileTimeToExif){
       if ((*ImageInfo)->numDateTimeTags){
            ...
            if (ExifTimeSet){
                ...
                if (FileTimeToExif){
                    FileTimeAsString((*ImageInfo)->DateTime,*ImageInfo);
                }
                if (DateSetChars){
...
skip_unixtime:         
    ...
    if (Modified){
        ...
        // Write the new file.
        WriteJpegFile(FileName,*ImageInfo);
    ...
    if (RenameToDate){
        DoFileRenaming(FileName,*ImageInfo);
    }
    DiscardData(*ImageInfo);
    return;
badtime:
